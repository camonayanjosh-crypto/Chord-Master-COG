...
const STORAGE_KEY = 'chordmaster_state';
...
const App: React.FC = () => {
  ...
  const navigate = (view: ViewType, payload?: { songId?: string; scheduleId?: string }) => {
    ...
    // Reset transposition state when switching songs
    if (view === 'view-song') {
  ...
  const handleAddSong = (e: React.FormEvent) => {
    ...
    setNewSong({ title: '', artist: '', originalKey: 'C', content: '' });
    setIsEditing(false);
  };
  ...
  const deleteSong = (id: string) => {
    if (confirm('Are you sure you want to delete this song?')) {
      setState(prev => ({ ...prev, songs: prev.songs.filter(s => s.id !== id) }));
    }
  };
  ...
  const renderDashboard = () => (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
          <h2 className="text-xl font-bold mb-6">Song Library</h2>
          <div className="space-y-4">
             <div className="flex items-center space-x-4 p-4 rounded-xl bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400">
                <Music className="w-6 h-6" />
                <div>
                  <p className="text-sm font-medium opacity-80 uppercase tracking-tighter">Total Songs</p>
                  <p className="text-2xl font-bold leading-none">{state.songs.length}</p>
                </div>
             </div>
             <button 
              onClick={() => navigate('songs')}
              className="w-full py-3 border-2 border-dashed border-gray-200 dark:border-gray-600 rounded-xl text-gray-400 hover:text-gray-600 hover:border-gray-300 dark:hover:text-gray-200 transition-all font-medium"
             >
               Explore Songs
             </button>
          </div>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button 
          onClick={() => {
            setIsEditing(false);
            setNewSong({ title: '', artist: '', originalKey: 'C', content: '' });
            navigate('songs'); // Just refresh view, but we'll show form below
        ...
        {filteredSongs.map(song => (
          <div key={song.id} className="bg-white dark:bg-gray-800 p-5 rounded-2xl border border-gray-100 dark:border-gray-700 hover:shadow-md transition-shadow group flex flex-col justify-between">
            <div className="flex justify-between items-start">
              <div onClick={() => navigate('view-song', { songId: song.id })} className="cursor-pointer">
        ...
        <form onSubmit={handleAddSong} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="md:col-span-1 space-y-4">
              <div>
                ...
                <input 
                  required
                  type="text" 
                  className="w-full bg-gray-50 dark:bg-gray-900 border-gray-200 dark:border-gray-700 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-blue-500 outline-none"
                  value={newSong.title}
              ...
              <div>
                ...
                <select 
                  className="w-full bg-gray-50 dark:bg-gray-900 border-gray-200 dark:border-gray-700 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-blue-500 outline-none appearance-none"
                  value={newSong.originalKey}
                  onChange={e => setNewSong({...newSong, originalKey: e.target.value as Key})}
                >
            ...
            <div className="md:col-span-2">
              ...
              <textarea 
                required
                rows={10}
                placeholder="Paste from Ultimate Guitar or type here..."
                className="w-full bg-gray-50 dark:bg-gray-900 border-gray-200 dark:border-gray-700 rounded-xl px-4 py-4 focus:ring-2 focus:ring-blue-500 outline-none font-mono text-sm leading-relaxed"
          ...
          <div className="flex justify-end space-x-3">
            {isEditing && (
              <button 
                type="button" 
                onClick={() => { setIsEditing(false); setNewSong({ title: '', artist: '', originalKey: 'C', content: '' }); }}
            ...
            <button 
              type="submit" 
              className="bg-blue-600 hover:bg-blue-700 text-white px-8 py-2.5 rounded-xl font-bold shadow-lg shadow-blue-200 dark:shadow-none"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1 bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 h-fit">
          ...
          <form onSubmit={handleAddSchedule} className="space-y-4">
            <div>
              <label className="block text-sm font-bold mb-1 uppercase tracking-tighter">Event Name</label>
              <input 
                required
                type="text" 
                placeholder="Sunday Service"
                className="w-full bg-gray-50 dark:bg-gray-900 border-gray-200 dark:border-gray-700 rounded-xl px-4 py-2.5 outline-none focus:ring-2 focus:ring-blue-500"
                value={newSchedule.name}
                onChange={e => setNewSchedule({...newSchedule, name: e.target.value})}
              />
            </div>
            <div>
              <label className="block text-sm font-bold mb-1 uppercase tracking-tighter">Date</label>
              <input 
                required
                type="date" 
                className="w-full bg-gray-50 dark:bg-gray-900 border-gray-200 dark:border-gray-700 rounded-xl px-4 py-2.5 outline-none focus:ring-2 focus:ring-blue-500"
                ...
                {state.songs.map(song => (
                  <label key={song.id} className="flex items-center space-x-3 p-2 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-lg cursor-pointer transition-colors">
                    <input 
                      ...
                      checked={newSchedule.songIds.includes(song.id)}
                      onChange={(e) => {
                        const ids = e.target.checked 
                          ? [...newSchedule.songIds, song.id]
                          : newSchedule.songIds.filter(id => id !== song.id);
            ...
            <button 
              type="submit" 
              className="w-full bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-xl font-bold shadow-lg shadow-blue-200 dark:shadow-none transition-all"
            >
              Add Schedule
            </button>
          </form>
                ...
                <div className="flex items-center justify-between mt-6">
                  <div className="flex -space-x-2 overflow-hidden">
                    {sch.songIds.slice(0, 3).map((id, idx) => (
                      <div key={id} className="inline-block h-8 w-8 rounded-full ring-2 ring-white dark:ring-gray-800 bg-blue-100 dark:bg-blue-900 flex items-center justify-center text-[10px] font-bold text-blue-600 dark:text-blue-300">
                        {idx + 1}
                      </div>
                    ))}
                    {sch.songIds.length > 3 && (
      ...
      <div className="flex flex-col h-[calc(100vh-6rem)] animate-in zoom-in-95 duration-300">
        <header className="flex justify-between items-center mb-6 shrink-0">
          ...
          <div className="flex items-center space-x-3">
             <div className="relative group">
                ...
                <div className="absolute right-0 mt-3 w-56 bg-white dark:bg-gray-800 border-2 border-gray-100 dark:border-gray-700 rounded-2xl shadow-2xl z-50 opacity-0 group-hover:opacity-100 pointer-events-none group-hover:pointer-events-auto transition-all p-3 grid grid-cols-4 gap-2">
                  {ALL_KEYS.map(k => (
                    <button 
                      key={k} 
                      onClick={() => { setTargetKey(k); setUseNashville(false); }}
                      className={`p-3 rounded-xl text-sm font-black transition-all ${targetKey === k && !useNashville ? 'bg-blue-600 text-white shadow-md' : 'hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300'}`}
                    >
                      {k}
                    </button>
                  ))}
                  <button 
                    ...
                    className={`col-span-4 mt-2 p-3 rounded-xl text-lg font-black transition-all flex items-center justify-center space-x-2 ${useNashville ? 'bg-indigo-600 text-white shadow-md shadow-indigo-200' : 'bg-gray-50 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 hover:text-indigo-600'}`}
                  >
             ...
             <button 
              onClick={() => { setIsEditing(true); setNewSong(currentSong); navigate('songs'); }}
              className="bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 p-2.5 rounded-xl transition-colors"
             >
        ...
        <div className="flex-1 overflow-auto bg-white dark:bg-gray-800 rounded-3xl p-8 md:p-12 shadow-sm border border-gray-100 dark:border-gray-700">
        ...
        {/* Footer Navigation for Setlist */}
        {currentSchedule && (
          <footer className="mt-6 p-4 bg-gray-900 text-white rounded-2xl flex items-center justify-between shrink-0 shadow-lg">
            <button 
              disabled={!hasPrev}
              onClick={() => navigate('view-song', { songId: scheduleSongs[currentIndex - 1], scheduleId: currentSchedule.id })}
              className={`flex items-center space-x-2 px-4 py-2 rounded-xl transition-colors ${!hasPrev ? 'opacity-30' : 'hover:bg-gray-800'}`}
            >
              <ChevronLeft className="w-5 h-5" />
              <span className="font-bold">Previous Song</span>
            </button>
            <div className="hidden md:block">
              <span className="text-gray-400 text-sm font-bold uppercase tracking-widest">Setlist: {currentSchedule.name} ({currentIndex + 1}/{scheduleSongs.length})</span>
            </div>
            <button 
              disabled={!hasNext}
              onClick={() => navigate('view-song', { songId: scheduleSongs[currentIndex + 1], scheduleId: currentSchedule.id })}
              className={`flex items-center space-x-2 px-4 py-2 rounded-xl transition-colors ${!hasNext ? 'opacity-30' : 'hover:bg-gray-800'}`}
            >
              <span className="font-bold">Next Song</span>
    ...
    return (
      <div className="space-y-6 animate-in slide-in-from-top-4 duration-500">
        <header className="flex items-center space-x-4">
          <button onClick={() => navigate('schedules')} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-xl">
            <ChevronLeft className="w-6 h-6" />
          </button>
          <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
            <h2 className="text-xl font-bold mb-6">Setlist Order</h2>
            <div className="space-y-3">
              {schSongs.map((song, idx) => (
                <div 
                  key={song.id} 
                  onClick={() => navigate('view-song', { songId: song.id, scheduleId: currentSchedule.id })}
                  className="flex items-center justify-between p-4 rounded-xl border border-gray-100 dark:border-gray-700 hover:border-blue-300 dark:hover:border-blue-700 hover:bg-blue-50/50 dark:hover:bg-blue-900/10 cursor-pointer transition-all group"
                >
                  <div className="flex items-center space-x-4">
              ...
              {schSongs.length === 0 && (
                <p className="text-center py-12 text-gray-400 italic">No songs in this setlist yet.</p>
              )}
            </div>
          </div>

          <div className="bg-blue-600 rounded-2xl p-8 text-white shadow-xl shadow-blue-200 dark:shadow-none flex flex-col justify-between h-[400px]">
             <div>
                <Music className="w-12 h-12 mb-6 opacity-80" />
                <h2 className="text-3xl font-extrabold mb-4">Practice Mode</h2>
                <p className="text-blue-100 text-lg">Launch the setlist viewer to practice your songs with automated transposition and seamless transitions.</p>
             </div>
             {schSongs.length > 0 && (
               <button 
                onClick={() => navigate('view-song', { songId: schSongs[0].id, scheduleId: currentSchedule.id })}
                className="bg-white text-blue-600 font-extrabold py-4 px-8 rounded-xl shadow-lg hover:bg-blue-50 transition-colors flex items-center justify-center space-x-2"
               >
        ...
        <div className="p-6 flex items-center justify-between">
          <div className="flex items-center space-x-4">
            <div className="p-3 bg-gray-100 dark:bg-gray-700 rounded-xl">
              {state.theme === 'light' ? <Sun className="w-6 h-6 text-orange-500" /> : <Moon className="w-6 h-6 text-blue-400" />}
            </div>
            <div>
          ...
          <button 
            ...
            className="w-14 h-8 bg-gray-200 dark:bg-gray-700 rounded-full relative p-1 transition-colors"
          >
            <div className={`w-6 h-6 bg-white dark:bg-blue-500 rounded-full shadow-sm transition-transform transform ${state.theme === 'dark' ? 'translate-x-6' : ''}`} />
          </button>
        </div>
        ...
        <div className="p-6 flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">Permanently delete all songs and schedules</p>
            </div>
          </div>
          <button 
            onClick={() => {
            ...
            className="px-4 py-2 border border-red-200 dark:border-red-900 text-red-500 font-bold rounded-xl hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
          >
            Reset All
          </button>
        </div>
    ...
    <div className="flex min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-200">
      ...
      <aside className="fixed left-0 top-0 bottom-0 w-64 bg-white dark:bg-gray-800 border-r border-gray-100 dark:border-gray-700 p-6 z-50 flex flex-col hidden lg:flex">
        ...
        <div className="mt-auto pt-6 border-t border-gray-100 dark:border-gray-700">
          <p className="text-[10px] uppercase tracking-widest text-gray-400 font-bold mb-4">Developed with Gemini</p>
          <div className="flex items-center space-x-3 px-2">
            <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-blue-500 to-indigo-600" />
            <div className="overflow-hidden">
              <p className="text-sm font-bold truncate">Guest Musician</p>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-800 border-t border-gray-100 dark:border-gray-700 flex justify-around p-3 lg:hidden z-50 shadow-2xl">
        <button onClick={() => navigate('dashboard')} className={`p-2 rounded-xl ${state.currentView === 'dashboard' ? 'text-blue-600 bg-blue-50 dark:bg-blue-900/20' : 'text-gray-400'}`}>
          <LayoutDashboard className="w-6 h-6" />
        </button>
        <button onClick={() => navigate('schedules')} className={`p-2 rounded-xl ${state.currentView === 'schedules' ? 'text-blue-600 bg-blue-50 dark:bg-blue-900/20' : 'text-gray-400'}`}>
          <Calendar className="w-6 h-6" />
        </button>
        <button onClick={() => navigate('songs')} className={`p-2 rounded-xl ${state.currentView === 'songs' ? 'text-blue-600 bg-blue-50 dark:bg-blue-900/20' : 'text-gray-400'}`}>
          <Music className="w-6 h-6" />
        </button>
        <button onClick={() => navigate('settings')} className={`p-2 rounded-xl ${state.currentView === 'settings' ? 'text-blue-600 bg-blue-50 dark:bg-blue-900/20' : 'text-gray-400'}`}>
